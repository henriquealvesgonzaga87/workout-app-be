...
class TestDeleteUserErrorHandling:
    ...
    def test_delete_use_case_handles_request_error(self, mock_user_repository):
        ...
        with pytest.raises(RequestError) as exc_info:
            use_case.execute(id="invalid")

        assert "Expecting int" in str(exc_info.value)
        assert isinstance(exc_info.value, RequestError)

    def test_delete_use_case_handles_response_validation_error(self, mock_user_repository):
    ...
    def test_delete_use_case_handles_not_found_error_from_repository(self, mock_user_repository):
    ...
    def test_delete_use_case_handles_database_error_from_repository(self, mock_user_repository):
    ...
    def test_prepare_method_error_message_format(self, mock_user_repository):
        """Test that prepare method error messages are properly formatted."""
        use_case = DeleteUserUseCase(user_repository=mock_user_repository)

        with pytest.raises(RequestError) as exc_info:
            use_case.prepare(id=[1, 2, 3])  # List is invalid

        error_msg = str(exc_info.value)
        assert "Expecting int" in error_msg
        assert "list" in error_msg.lower()

    def test_delete_with_none_returns_error(self, mock_user_repository):
    ...
    def test_delete_with_empty_string_raises_error(self, mock_user_repository):
    ...
    def test_delete_with_dict_raises_error(self, mock_user_repository):
        """Test that passing dict as ID raises RequestError."""
        use_case = DeleteUserUseCase(user_repository=mock_user_repository)

        with pytest.raises(RequestError) as exc_info:
            use_case.prepare(id={"id": 1})

        assert "Expecting int" in str(exc_info.value)

    def test_delete_with_object_raises_error(self, mock_user_repository):
        ...
        with pytest.raises(RequestError):
            use_case.prepare(id=CustomObject())

    def test_error_message_contains_type_information(self, mock_user_repository):
        """Test that error message includes the type that was passed."""
        use_case = DeleteUserUseCase(user_repository=mock_user_repository)

        with pytest.raises(RequestError) as exc_info:
            use_case.prepare(id={"test": "dict"})

        error_msg = str(exc_info.value)
        assert "dict" in error_msg

    def test_request_error_vs_response_error_distinction(self, mock_user_repository):
    ...
    def test_multiple_error_types_in_sequence(self, mock_user_repository):
        """Test handling different error types in sequence."""
        use_case = DeleteUserUseCase(user_repository=mock_user_repository)

        # First error: RequestError
        with pytest.raises(RequestError):
            use_case.execute(id="not_int")
    ...
    def test_error_recovery_after_failed_delete(self, mock_user_repository):
        ...
        result = use_case.execute(id=2)
        assert result is True

    def test_database_error_during_preparation(self, mock_user_repository):
        ...
        for error_input in error_inputs:
            with pytest.raises(RequestError):
                use_case.prepare(id=error_input)

        # Float can be converted, so it won't raise an error
        result = use_case.prepare(id=3.14)
        assert result == 3

    def test_error_isolation_between_delete_calls(self, mock_user_repository):
    ...
    def test_prepare_error_with_boundary_values(self, mock_user_repository):
        """Test prepare method with boundary and edge case values."""
        use_case = DeleteUserUseCase(user_repository=mock_user_repository)

        # Valid boundary values
        assert use_case.prepare(id=0) == 0
        assert use_case.prepare(id=1) == 1
        assert use_case.prepare(id="0") == 0
        assert use_case.prepare(id="999999999") == 999999999

    def test_custom_error_message_in_request_error(self, mock_user_repository):
        """Test that RequestError includes helpful error messages."""
        use_case = DeleteUserUseCase(user_repository=mock_user_repository)

        with pytest.raises(RequestError) as exc_info:
            use_case.prepare(id={"invalid": "dict"})
